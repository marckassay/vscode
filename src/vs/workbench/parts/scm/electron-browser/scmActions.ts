...
export function isSCMViewletFocussed(viewletService: IViewletService): boolean {
	let activeViewlet = viewletService.getActiveViewlet();
	let activeElement = document.activeElement;
	return activeViewlet && activeViewlet.getId() === Constants.VIEWLET_ID && activeElement && DOM.isAncestor(activeElement, (<SCMViewlet>activeViewlet).getContainer().getHTMLElement());
}
...
export class ToggleAnnotateAction extends Action {
	...
	public run(): TPromise<any> {
		let scmViewlet = <SCMViewlet>this.viewletService.getActiveViewlet();
		scmViewlet.toggleAnnotate();
...
export class OpenSCMViewletAction extends ToggleViewletAction {
	...
	public run(): TPromise<any> {
		...
		const scmViewletWasOpen = activeViewlet && activeViewlet.getId() === Constants.VIEWLET_ID;
...
export class FindInFilesAction extends FindOrReplaceInFilesAction {
	...
	constructor(id: string, label: string, @IViewletService viewletService: IViewletService) {
		super(id, label, viewletService, false, true, false);
	}
}
...
export class ReplaceInFilesAction extends FindOrReplaceInFilesAction {
	...
	constructor(id: string, label: string, @IViewletService viewletService: IViewletService) {
		super(id, label, viewletService, true, false, true);
	}
}
**/
...
export class RefreshAction extends Action {
	...
	constructor(private viewlet: SCMViewlet) {
		super('refresh');
